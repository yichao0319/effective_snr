...
%
% Example:
%   sim_rate_adapt3(1, 'preambleSNR', 'Thresholding', 'currPkt')
%
...
function [tput] = sim_rate_adapt3(file_ind, method_SNR, method_RA, method_PRED, threshold)
    ...
    elseif strcmp(method_RA, Thresholding)
        pprs_BPSK = cal_prr_from_threshold(pred_bers_BPSK, 1);
        pprs_QPSK = cal_prr_from_threshold(pred_bers_QPSK, 1);
        pprs_16QAM = cal_prr_from_threshold(pred_bers_16QAM, threshold_BER(3));
        pprs_64QAM = cal_prr_from_threshold(pred_bers_64QAM, threshold_BER(4));
        pprs = [pprs_BPSK; pprs_QPSK; pprs_16QAM; pprs_64QAM];

        tput = cal_tput(pprs, actual_bers, threshold, TPUT_TABLE);
    elseif strcmp(method_RA, Probability)
        num_bits_per_pkt = [num_bits_per_pkt_BPSK; num_bits_per_pkt_QPSK; num_bits_per_pkt_16QAM; num_bits_per_pkt_64QAM];
        k = [0; 0; 0; 0];
        pprs = cal_probability(pred_bers, pred_stdevs, num_bits_per_pkt, k);
...
function [prrs] = cal_probability(bers, stdevs, num_bits_per_pkt, k)
    ...
    %% ------------------------
    % DEBUG
    % dlmwrite([output_dir 'tmp_pprs.dat'], prrs);
